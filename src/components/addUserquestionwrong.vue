			...
			<div class="row">
				<div class="col-xl-9 col-lg-10 col-md-12 col-sm-12 mx-auto">
					<div class="tm-bg-primary-dark tm-block tm-block-h-auto">
						<div class="row">
							<div class="col-12">
								<h2 class="tm-block-title d-inline-block">新增用户错题数据</h2>
							</div>
						</div>
						<div class="row tm-edit-product-row">
							<div class="col-xl-6 col-lg-6 col-md-12">
								<!-- <form action="" class="tm-edit-product-form"> -->
                                        ...
                                        <input id="questionId" type="text" v-validate="'required'" name="试题id" v-model="userquestionwrong.questionId" class="form-control validate" />
                                        <span class="validateErrorSpan">{{ errors.first('试题id') }}</span>
                                    </div>
                                                                                                                                            <div class="form-group mb-3">
                                        <label for="createDate">错题时间</label>
                                        <input id="createDate" type="text" v-validate="'required'" name="错题时间" v-model="userquestionwrong.createDate" class="form-control validate" />
                                        <span class="validateErrorSpan">{{ errors.first('错题时间') }}</span>
                                    </div>
                                                                                                                                            <div class="form-group mb-3">
                                        <label for="wrongCount">错题次数</label>
                                        <input id="wrongCount" type="text" v-validate="'required'" name="错题次数" v-model="userquestionwrong.wrongCount" class="form-control validate" />
                                        <span class="validateErrorSpan">{{ errors.first('错题次数') }}</span>
                                    </div>
                                                                                                                                            <div class="form-group mb-3">
                                        <label for="questionTypeId">试题类型  1选择题 2判断题</label>
                                        <input id="questionTypeId" type="text" v-validate="'required'" name="试题类型  1选择题 2判断题" v-model="userquestionwrong.questionTypeId" class="form-control validate" />
                                        <span class="validateErrorSpan">{{ errors.first('试题类型  1选择题 2判断题') }}</span>
                                    </div>
                                                                                                                                            <div class="form-group mb-3">
                                        <label for="questionJobTypeId">试题所属得岗位类型id</label>
                                        <input id="questionJobTypeId" type="text" v-validate="'required'" name="试题所属得岗位类型id" v-model="userquestionwrong.questionJobTypeId" class="form-control validate" />
							...
							<div class="col-12">
								<button @click="addUserquestionwrong" class="btn btn-primary btn-block text-uppercase">确认添加</button>
							</div>
							<!-- </form> -->
			...
			addUserquestionwrong: function() {
				this.$validator.validate().then(valid => {
					if (!valid) {
						...
						this.$http.post('/msbd/addUserquestionwrong', that.userquestionwrong).then(res => {
