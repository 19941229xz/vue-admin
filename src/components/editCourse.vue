			...
			<div class="row">
				<div class="col-xl-9 col-lg-10 col-md-12 col-sm-12 mx-auto">
					<div class="tm-bg-primary-dark tm-block tm-block-h-auto">
						<div class="row">
							<div class="col-12">
								<h2 class="tm-block-title d-inline-block">编辑课程数据</h2>
							</div>
						</div>
						<div class="row tm-edit-product-row">
							...
							<div class="col-xl-6 col-lg-6 col-md-12 mx-auto mb-4">
								...
								<div class="custom-file mt-3 mb-3">
									<input @change="addImage" id="coursePic1" type="file" style="display:none;" />
								...
								<div class="custom-file mt-3 mb-3">
									<input @change="addImage" id="courseQrcode" type="file" style="display:none;" />
							...
							<div class="col-12">
								<button @click="saveCourse" class="btn btn-primary btn-block text-uppercase">保存</button>
								<button @click="toCourseList" class="btn btn-primary btn-block text-uppercase">返回课程列表</button>
							</div>
							<!-- </form> -->
						</div>
					</div>
			...
			saveCourse: function() {
				this.$validator.validate().then(valid => {
					if (!valid) {
						...
						this.$http.put('/msbd/updateCourse', that.course).then(res => {
							if (res.data.code == 200) {
								this.course = res.data.content
								this.$infoMsg('保存成功')
								this.$log(res)
							} else {
								this.$errMsg('课程数据修改失败')
							}
							this.getCourseById(this.itemId)
						}).catch(err => {
							this.$log(err)
							this.$errMsg('课程数据修改失败')
							this.getCourseById(this.itemId)
